...
test.describe('ログイン機能', () => {
  ...
  test('指揮本部アカウントでログイン', async ({ page }) => {
    ...
    // リダイレクト確認
    await expect(page).toHaveURL(/\/command/, { timeout: 20000 })
  ...
  test('トリアージ担当アカウントでログイン', async ({ page }) => {
    ...
    // トリアージ入力画面へリダイレクト
    await expect(page).toHaveURL(/\/triage\/scan/, { timeout: 20000 })
  })
  ...
  test('搬送担当アカウントでログイン', async ({ page }) => {
    ...
    // 搬送画面へリダイレクト
    await expect(page).toHaveURL(/\/transport/, { timeout: 20000 })
  })
