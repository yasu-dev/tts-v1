...
本機能の RFID 機能は、想定される運用形態として以下 2 通りを想定する。設計はこの両方に対応する。

| シナリオ                                   | 担当・端末                                                                                           | 運用詳細                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **① 現場で直接登録**                       | タッグ隊 (TRI ロール)<br>iPad / Windows PC                                                           | タッグ隊が現場でトリアージ実施 → その場で `/triage/scan` を開き、タグを読み取って傷病者情報を登録する。タブレット運用時は QR + 手入力、Windows PC + RFID リーダー接続時は QR / RFID / 手入力の 3 経路。**Red / Yellow / Black 傷病者は移動困難なためこの運用**                                                      |
| **② タッグ配布と登録の分業** (RFID 主用途) | タッグ隊 (現場): タッグ貼付のみ<br>システム登録者 (集合場所): TRI ロール、Windows PC + RFID リーダー | ① タッグ隊は現場で物理 RFID タッグを傷病者に貼付するだけ (システム登録なし)。② 自力で歩ける傷病者 (Green 想定) は集合場所まで移動。③ 集合場所のシステム登録者が `/triage/scan` を Windows PC で開き、RFID リーダーで来た人のタッグを順次読み取って必要最低限の情報を登録。**軽症 (Green) 傷病者の効率的処理に最適** |

運用設計上の留意点 (本機能のスコープ外、運用ルールで担保):

- **同時読取防止**: TFU-RW811A は UHF Inventory モードで複数タッグを同時返却し得るため、ブリッジ側で「最初の 1 件のみ採用」を実装済 (§3.5 E1)。運用としては「1 人ずつ前に出てかざす」ルールを設けて誤読を防ぐ
- **読取距離**: TFU-RW811A は近接型 (数十 cm〜1m 程度)。タッグの装着位置 (首掛け / 腕巻き / 服に貼付) を運用で固定する
- **タッグ事前配布管理**: 未割当タッグの物理在庫、紛失時対応、回収済タッグの再利用可否
- **集合場所の通信**: システム登録者 PC が Supabase に常時接続できる回線必須 (4G/5G オフライン耐性は本機能スコープ外)
- **同一タッグ二重登録**: DB の `UNIQUE(event_id, tag_number)` 複合制約で検知 (§3.4)
