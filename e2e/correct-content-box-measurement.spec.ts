...
test.describe('実際のコンテンツボックス幅測定', () => {
  ...
  test('白いコンテンツカードの幅を正確に測定', async () => {
    ...
    for (const screen of screens) {
      ...
      const measurements = await page.evaluate(() => {
        ...
        intelligenceCards.forEach((card, index) => {
          ...
          const isWhiteCard = backgroundColor.includes('rgb(255, 255, 255)') || 
      ...
      if (measurements.whiteCards.length > 0) {
        console.log(`  白いカードの幅:`);
        measurements.whiteCards.forEach((card: any, i: number) => {
          console.log(`    カード${i + 1}: ${card.width}px (位置: ${card.left}px-${card.right}px)`);
        });
        
        const uniqueWidths = Array.from(new Set(measurements.whiteCardWidths));
    ...
    const allWhiteCardWidths: number[] = [];
    Object.entries(allMeasurements).forEach(([screenName, data]: [string, any]) => {
    ...
    const allUniqueWidths = Array.from(new Set(allWhiteCardWidths));
    console.log(`\n総合結果:`);
    ...
    if (allUniqueWidths.length === 1) {
      ...
      const mostCommonWidth = Object.keys(widthCounts).reduce((a, b) => 
        widthCounts[parseInt(a)] > widthCounts[parseInt(b)] ? a : b
      );
