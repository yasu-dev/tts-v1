        ...
        vital_signs_records: {
          first: {
            ...
            respiratory_rate: vitalSigns.respiratory_rate
              ? parseInt(vitalSigns.respiratory_rate)
              : undefined,
      ...
      <main className="mx-auto max-w-4xl p-6">
        ...
        <div className="mb-6">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span
              className={`text-sm font-bold ${currentStep === 'vitals' ? 'text-emerald-600' : 'text-gray-400'}`}
            >
              3. バイタル
            </span>
            <span
              className={`text-sm font-bold ${currentStep === 'info' ? 'text-emerald-600' : 'text-gray-400'}`}
            >
              4. 患者情報
            </span>
            <span
              className={`text-sm font-bold ${currentStep === 'confirm' ? 'text-emerald-600' : 'text-gray-400'}`}
            >
              5. 確認
            </span>
          </div>
              ...
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  {contactPoints.length > 0 && (
                    <p className="mt-1 text-xs text-gray-500">登録済み: {contactPoints.length}件</p>
                  )}
                </div>
                <button
                ...
                >
                  {isContactPointSectionExpanded ? <span>▲ 折りたたむ</span> : <span>▼ 展開</span>}
                </button>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="whitespace-nowrap rounded-lg bg-purple-600 px-4 py-2 font-bold text-white transition hover:bg-purple-700"
                    >
                      管理
                    </button>
                  </div>
                  {contactPoints.length > 0 && (
                    <div className="rounded border border-gray-200 bg-gray-50 p-3">
                      <p className="mb-2 text-xs text-gray-600">登録済み接触地点:</p>
                      <div className="flex flex-wrap gap-1">
                        {contactPoints.map((point, index) => (
                          <span
                            key={index}
                            className="inline-block rounded bg-purple-100 px-2 py-1 text-xs font-bold text-purple-800"
                          >
                            {point}
                          </span>
                        ))}
        ...
        {currentStep === 'vitals' && (
          <div className="rounded-lg bg-white p-6 shadow-md">
            <div className="mb-6">
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  ...
                  className="rounded-lg bg-gray-300 px-4 py-2 text-sm font-bold text-gray-700 transition hover:bg-gray-400"
                >
                  最初からやり直す
                </button>
              </div>
              {triageResult && (
                  ...
                  style={{
                    backgroundColor:
                      ['black', 'red', 'yellow', 'green'][
                        ['black', 'red', 'yellow', 'green'].indexOf(triageResult.category)
                      ] === 'black'
                        ? '#000'
                        : ['black', 'red', 'yellow', 'green'][
                              ['black', 'red', 'yellow', 'green'].indexOf(triageResult.category)
                ...
                >
                  判定:{' '}
                  {triageResult.category === 'black'
                    ? '黒（死亡）'
                    : triageResult.category === 'red'
                      ? '赤（重症）'
                      : triageResult.category === 'yellow'
                        ? '黄（中等症）'
                        : '緑（軽症）'}
                </div>
              )}
            ...
            <form onSubmit={handleVitalSubmit} className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-bold">判定者名</label>
                <input
                  type="text"
              ...
              <div>
                <label className="mb-2 block text-sm font-bold">判定場所</label>
                <input
                  type="text"
                  value={vitalSigns.judgment_location}
                  onChange={(e) =>
                    setVitalSigns({ ...vitalSigns, judgment_location: e.target.value })
                  }
                  className="input"
                  placeholder="例: 現場"
                />
              ...
              <div>
                <label className="mb-2 block text-sm font-bold">判定時間</label>
                <input
                  type="time"
              ...
              <div>
                <label className="mb-2 block text-sm font-bold">意識レベル</label>
                <select
              ...
              <div>
                <label className="mb-2 block text-sm font-bold">呼吸数（回/分）</label>
                <input
                  type="number"
              ...
              <div>
                <label className="mb-2 block text-sm font-bold">脈拍数（回/分）</label>
                <input
                  type="number"
              ...
              <div>
                <label className="mb-2 block text-sm font-bold">血圧（mmHg）</label>
                <div className="flex gap-2">
                  <input
                    type="number"
                    value={vitalSigns.blood_pressure_systolic}
                    onChange={(e) =>
                      setVitalSigns({ ...vitalSigns, blood_pressure_systolic: e.target.value })
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setVitalSigns({ ...vitalSigns, blood_pressure_diastolic: e.target.value })
                    }
                    className="input flex-1"
                    placeholder="拡張期"
                  />
              ...
              <div>
                <label className="mb-2 block text-sm font-bold">体温（°C）【任意】</label>
                <input
                  type="number"
                  step="0.1"
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 rounded-lg bg-gray-300 px-6 py-3 font-bold text-gray-700 hover:bg-gray-400"
                >
                  戻る
                </button>
        ...
        {currentStep === 'info' && (
          <div className="rounded-lg bg-white p-6 shadow-md">
            ...
            <form onSubmit={handleInfoSubmit} className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-bold">推定年齢</label>
                <input
                  type="number"
              ...
              {contactPoints.length > 0 && (
                <div>
                  <label className="mb-2 block text-sm font-bold">接触地点【任意】</label>
                  <select
                    value={contactPoint}
                  ...
                  <p className="mt-1 text-xs text-gray-500">現場内での患者発見位置を選択できます</p>
                </div>
              )}

              <div>
                <label className="mb-2 block text-sm font-bold">メモ・所見</label>
                <textarea
                  value={notes}
          ...
          <div className="rounded-lg bg-white p-6 shadow-md">
            ...
            <div className="space-y-4">
              <div className="border-b pb-3">
                ...
                <p className="text-lg font-bold">{tagNumber}</p>
              </div>
              ...
              {triageResult && (
                <div className="border-b pb-3">
                  ...
                  <p className="mt-1 text-sm text-gray-600">理由: {triageResult.reasoning}</p>
                </div>
              )}
              ...
              <div className="border-b pb-3">
                ...
                <ul className="mt-1 text-sm">
                  ...
                  {vitalSigns.judgment_location && (
                    <li>判定場所: {vitalSigns.judgment_location}</li>
                  )}
                  ...
                  {(vitalSigns.blood_pressure_systolic || vitalSigns.blood_pressure_diastolic) && (
                    <li>
                      血圧: {vitalSigns.blood_pressure_systolic || '-'}/
                      {vitalSigns.blood_pressure_diastolic || '-'} mmHg
                    </li>
                  )}
              ...
              {contactPoint && (
                <div className="border-b pb-3">
                  ...
                  <p className="mt-1 text-sm font-bold text-purple-700">{contactPoint}</p>
                </div>
              )}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {uploadedImages.map((image, index) => (
                      <div key={image.id} className="relative">
                        ...
                        />
                        <span className="absolute right-1 top-1 rounded bg-black bg-opacity-70 px-2 py-1 text-xs text-white">
                          {image.type === 'wound'
                            ? '外傷'
                            : image.type === 'scene'
                              ? '現場'
            ...
            {error && (
              <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-4">
                <p className="whitespace-pre-line font-medium text-red-800">{error}</p>
              </div>
            )}
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex-1 rounded-lg bg-gray-300 px-6 py-3 font-bold text-gray-700 hover:bg-gray-400 disabled:opacity-50"
              >
                戻る
              </button>
              <button
              ...
              >
                {loading ? '登録中...' : !networkStatus ? 'オフライン' : '登録する'}
              </button>
            </div>
          </div>
        )}
