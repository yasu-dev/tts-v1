...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('FedEx - 対象商品ID:', productIds);
      
      // デフォルトのピッキング用ロケーションを取得または作成
      console.log('FedEx - PICK-01ロケーション確認中...');
      ...
      if (!pickingLocation) {
        console.log('FedEx - PICK-01が存在しません。作成中...');
        try {
          ...
          console.log('FedEx - ✅ PICK-01ロケーション作成完了:', pickingLocation.id);
        } catch (locationError) {
          console.error('FedEx - ❌ PICK-01作成エラー:', locationError);
          throw locationError;
        }
      } else {
        console.log('FedEx - ✅ PICK-01ロケーション既存確認:', pickingLocation.id);
      }
      
      console.log('FedEx - 商品ステータス更新中...');
      try {
        ...
        console.log('FedEx - ✅ 商品ステータス更新完了:', updateResult.count, '件更新');
      } catch (updateError) {
        console.error('FedEx - ❌ 商品ステータス更新エラー:', updateError);
        throw updateError;
      }
      ...
      await prisma.shipment.create({
        data: {
          ...
          notes: `FedEx配送ラベル生成済み - Service: ${service}`,
        }
      });
