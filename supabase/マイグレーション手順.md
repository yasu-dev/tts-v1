...
> **⚠️ 本ドキュメントに含まれる `DROP COLUMN` SQL は本番 DB に対して実行禁止**
>
> 本番（`triage-tag-system` プロジェクト）に対するスキーマ変更は forward-only ポリシー（追加のみ・カラム削除や `NOT NULL` 制約追加は禁止）に従う。これは `.github/workflows/sql-guard.yml` の CI で機械的に検証される。
>
> 本ドキュメントの `DROP COLUMN` 群はローカル／開発用プロジェクトでのスキーマ再構築用。本番 DB に流すとロールバック後の旧コードがエラーになる可能性があり、データ損失リスクがあります。詳細は [`docs/runbooks/rollback.md`](../docs/runbooks/rollback.md) を参照。
...
## 新しいテーブルを作るときの権限（2026-10-30 以降は必須）

2026-10-30 から、Supabase は `public` に新しく作るテーブル・シーケンスへ Data API（supabase-js / PostgREST）の権限を自動で付けなくなる。既存テーブルの権限はそのまま残る。付け忘れると、そのテーブルだけ API から `permission denied`（42501）になる。`service_role` も例外ではない。

新しいテーブルを作る SQL には、同じファイルの中で次をそろえる。

```sql
-- 必要なロールに、必要な操作だけを付ける（例）
grant select, insert, update, delete on public.<table> to authenticated;
grant select, insert, update, delete on public.<table> to service_role;
-- 未ログインで読む必要がある表だけ
grant select on public.<table> to anon;
-- serial / bigserial を使う場合はシーケンスも
grant usage, select on sequence public.<table>_id_seq to authenticated, service_role;

alter table public.<table> enable row level security;
-- ポリシーを定義する
```

- 既存テーブルに一括で `grant all` はしない（必要以上の権限になる）
- 適用後は、ロールごと（authenticated / anon / service_role）に API から許可・拒否を実際に操作して確かめる
- 公式: [Securing your API](https://supabase.com/docs/guides/api/securing-your-api)
