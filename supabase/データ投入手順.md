...
> **⚠️ 本番 DB に対して実行禁止**
>
> 本ドキュメントに含まれる `TRUNCATE TABLE ... CASCADE;` および `DROP TABLE` の手順は、ローカル／開発用 Supabase プロジェクトでのみ実行すること。本番（`triage-tag-system` プロジェクト）に対して実行するとデータが全消失し、ロールバック手順では復旧できません。
>
> 本番に対するスキーマ変更は forward-only ポリシー（追加のみ）に従い、`.github/workflows/sql-guard.yml` の CI チェックを通過する SQL のみが許可されます。詳細は [`docs/runbooks/rollback.md`](../docs/runbooks/rollback.md) を参照。
